    ...
    $.ajax({
      ...
      success : function(data){
        var element =   "<li class = 'cardReplyList'>"+
                          ...
                          "<p>"+
                            "<div class='post-add-icon inline-items sc_reply' id = "+ data[0].sc_r_no+">"+
                              "<svg class='olymp-heart-icon sc_r_nice'><use xmlns:xlink='http://www.w3.org/1999/xlink' xlink:href='/static/icons/icons.svg#olymp-heart-icon'></use></svg>"+
                              "<span class = 'sc_r_niceNum'>"+ data[0].niceNum +"</span>"+
                            "</div>"+
                          "</p>"+
                        "</li>";
      ...
      success : function(data){
        ...
        for(var i = 0 ; i < showReplyNum ; i++){
          element += "<li class = 'cardReplyList'>"+
                      ...
                      "<p>"+
                        "<div class='post-add-icon inline-items sc_reply' id = "+ data[data.length-(showReplyNum-i)].sc_r_no+">"+
                          ...
                          "<span class = 'sc_r_niceNum'>"+ data[data.length-(showReplyNum-i)].niceNum +"</span>"+
                        "</div>"+
                      "</p>"+
                    "</li>"
        }
    ...
    $.ajax({
      ...
      success : function(data){
        if(data == 'err'){
          ...
          $('#'+sc_r_no).children(".sc_r_niceNum").remove();
          var element = "<span class = 'sc_r_niceNum'>"+ data[0].niceNum +"</span>";
          $('#'+sc_r_no).append(element);
         }
      },error : function(err){
        console.log(err);
      }
