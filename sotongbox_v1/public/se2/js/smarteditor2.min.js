...
nhn.HuskyRange = jindo.$Class({
  ...
  pasteHTML: function(g, d) {
    var j = this._document.createElement("DIV");
    j.innerHTML = g;
    if (!j.firstChild) {
      this.deleteContents();
      return
    ...
    var o = n.placeStringBookmark();
    var r = n.getLineInfo(),
      q = r.oStart,
      h = r.oEnd;
    if (q.oLineBreaker && q.oLineBreaker.nodeName === "P" && (d || n.rxHasBlock.test(g))) {
      var e = q.oLineBreaker.parentNode,
        b = q.oLineBreaker.nextSibling;
      n.deleteContents();
      if (q.oLineBreaker === h.oLineBreaker) {
        var p = n.getStringBookmark(o);
        n.setEndNodes(p, h.oLineBreaker);
        var f = n.extractContents(),
          c = f.firstChild;
        if (b) {
          e.insertBefore(f, b)
        } else {
      ...
      j.style.cssText = q.oLineBreaker.style.cssText.replace(this._rxTextAlign, "");
      j.align = q.oLineBreaker.align;
      if (b) {
        e.insertBefore(j, b)
      } else {
        e.appendChild(j)
      ...
      this._removeEmptyP(this._getPrevElement(j));
      var a = this._getNextElement(j);
      if (a) {
        var m = this._getNextElement(a);
        if (m && this._removeEmptyP(a)) {
          a = m
        }
      } else {
        a = this._document.createElement("P");
        a.style.cssText = q.oLineBreaker.style.cssText;
        a.align = q.oLineBreaker.align;
        e.appendChild(a)
      }
      if (a.innerHTML === "") {
        a.innerHTML = (jindo.$Agent().navigator().ie && jindo.$Agent().navigator().version > 8) ? "\u200B" : "\uFEFF"
      }
      this.selectNodeContents(a);
      this.collapseToStart();
      if (jindo.$Agent().navigator().ie && jindo.$Agent().navigator().version < 9) {
        o = this.placeStringBookmark();
      ...
      this.collapseToStart();
      while (j.lastChild) {
        this.insertNode(j.lastChild)
      }
      this.setEndNodes(k, l);
      n.moveToBookmark(o);
      n.deleteContents();
      n.removeStringBookmark(o)
    }
...
nhn.husky.SE2M_Toolbar = jindo.$Class({
  ...
  $ON_ALERT: function(b, a) {
    if (this._elAlertLayer) {
      a = a || {};
      this._elAlertTxts.innerHTML = b || "";
      this._elAlertOk.onclick = jindo.$Fn(this._hideAlert, this).bind(a.fOkCallback);
      this._elAlertClose.onclick = jindo.$Fn(this._hideAlert, this).bind(a.fCloseCallback);
      if (a.fCancelCallback) {
        this._elAlertCancel.onclick = jindo.$Fn(this._hideAlert, this).bind(a.fCancelCallback);
  ...
  _assignHTMLElements: function(c) {
    ...
    this._elAlertLayer = jindo.$$.getSingle(".se2_alert_wrap", c);
    if (this._elAlertLayer) {
...
nhn.husky.SE_EditingAreaManager = jindo.$Class({
  ...
  _assignHTMLElements: function(a) {
    ...
    var b = function() {};
    this.elLoadingLayer = jindo.$$.getSingle(".se2_content_loading", a);
    if (!this.elLoadingLayer) {
      this.$ON_SHOW_LOADING_LAYER = b;
      this.$ON_HIDE_LOADING_LAYER = b
    }
  },
  ...
  $ON_SHOW_EDITING_AREA_COVER: function(a) {
  ...
  _convertLastBrToNbsp: function() {
    var h = this.oApp.getWYSIWYGDocument().body,
      k, r, p, l, e, g, m, j, c, s, d, f, a = document.createTextNode("\u00A0"),
      q, b;
    k = jindo.$$("br:last-child", h, {
    ...
    for (var n = 0, o = k.length; n < o; n++) {
      ...
      b = null;
      r = k[n];
      l = this._findNextSiblingRecursive(r, {
        isReverse: true
      });
      if (!(l && (l.nodeType === 1))) {
        continue
      ...
      p = l.nextSibling;
      g = this._findNextSiblingRecursive(p);
      if (g) {
        continue
      }
      if (l.tagName.toUpperCase() === "IMG") {
        j = l
      } else {
        m = jindo.$$("img:last-child", l, {
          oneTimeOffCache: true
        });
        if (m.length > 0) {
          j = m[m.length - 1]
      ...
      if (j) {
        e = this._findNextSiblingRecursive(j);
        if (e == p) {
          s = j.parentNode;
          if (!s) {
            continue
          }
          c = jindo.$Element(j).width(), s = j.parentNode, d = jindo.$Element(s).width();
          f = !(c === d)
        } else {
          f = true
        }
      ...
      if (f) {
        ...
        this._recursiveRemoveChild(r)
      }
    }
  },
  _findNextSiblingRecursive: function(c, f) {
    var h = c,
      b, e, g = (f && f.isReverse) ? true : false,
      a = new RegExp("^(P|DIV)$", "i"),
      d = new RegExp("^(TD|BODY)$", "i");
    while (!b) {
      if (g) {
        b = h.previousSibling
      } else {
  ...
  _recursiveRemoveChild: function(a) {
    var b = a.parentNode,
      d = a,
      c;
    do {
      b.removeChild(d)
    } while ((c = b.childNodes) && (c.length == 0) && (d = b) && (b = b.parentNode))
  },
...
nhn.husky.SE_EditingArea_WYSIWYG = jindo.$Class({
  ...
  _dbToIrDOM: function(a) {
    nhn.husky.SE2M_Utils.removeInvalidFont(a);
    nhn.husky.SE2M_Utils.convertFontToSpan(a)
  },
  ...
  _setAutoResize: function() {
    ...
    if (this._nContainerHeight !== d) {
      this._nContainerHeight = d;
  ...
  _backspace: function(b) {
    var a = this._prepareBackspaceDelete(true);
    if (!a) {
      return
    }
    if (this._removeUnremovable(a, true)) {
      b.stop()
    }
  },
  _delete: function(d) {
    var b = this._prepareBackspaceDelete(false);
    if (!b) {
      return
    }
    if (this._removeUnremovable(b, false)) {
      d.stop()
    } else {
      if (b.nodeType === 3) {
        var a = this.oApp.getSelection().getLineInfo(),
          c = a.oEnd.oLineBreaker,
          e = c && c.nextSibling;
        this._removeWrongSpan(e)
      } else {
        this._removeWrongSpan(b)
      }
    }
  },
  _prepareBackspaceDelete: function(a) {
    var b = this.oApp.getSelection();
    if (!b.collapsed) {
      return
    }
    var c = b.getNodeAroundRange(a, false);
    if (this._isLineFeed(c)) {
      c = a ? b._getPrevNode(c) : b._getNextNode(c)
    }
    this._clearCursorHolderValue(c);
    return c
  },
  _isLineFeed: function(a) {
    return (a && a.nodeType === 3 && /^[\n]*$/.test(a.nodeValue))
  },
  _clearCursorHolderValue: function(a) {
    if (a && a.nodeType === 3 && (a.nodeValue === "\u200B" || a.nodeValue === "\uFEFF")) {
      a.nodeValue = ""
    }
  },
  _removeUnremovable: function(d, b) {
    var a = false;
    if (!d) {
      return false
    }
    if (d.nodeName === "TABLE") {
      ...
      if (d.nodeName === "DIV") {
        var c = b ? d.lastChild : d.firstChild;
        if (!c) {
          d.parentNode.removeChild(d);
          a = true
        } else {
          if (c.nodeName === "TABLE") {
            ...
            if (c.nodeType === 1 && jindo.$S(c.innerHTML).trim() == "") {
              d.removeChild(c);
              a = true
            }
          }
  ...
  _removeWrongSpan: function(b) {
    if (b && b.nodeName === "SPAN" && b.firstChild && b.firstChild.nodeName === "P") {
      var a = b.parentNode;
      while (b.firstChild) {
        a.insertBefore(b.firstChild, b)
      }
      a.removeChild(b)
    }
  },
  ...
  $ON_PASTE_HTML: function(b, f, j) {
    ...
    c = f || this.oApp.getSelection();
    c.pasteHTML(b, j.bBlock);
    if (!h.ie) {
      e = c.placeStringBookmark();
      this.oApp.getWYSIWYGDocument().body.innerHTML = this.oApp.getWYSIWYGDocument().body.innerHTML;
      c.moveToBookmark(e);
      c.collapseToEnd();
      c.select();
      c.removeStringBookmark(e);
    ...
    if (b.indexOf("<img") > -1) {
      ...
      if (l.nodeType === 1 && l.tagName === "P") {
        g = jindo.$Element(l).child(function(m) {
          return (m.$value().nodeType === 1 && m.$value().tagName === "IMG")
        }, 1);
        if (g.length > 0) {
          d = g[g.length - 1].$value();
          k = d.nextSibling;
          while (k) {
            a = k.nextSibling;
  ...
  initIframe: function() {
    try {
      ...
      var c = (!!nhn.husky.SE2M_Configuration.SE2M_CSSLoader && nhn.husky.SE2M_Configuration.SE2M_CSSLoader.sCSSBaseURI) ? nhn.husky.SE2M_Configuration.SE2M_CSSLoader.sCSSBaseURI : "";
      ...
      if (c) {
        var h = c;
        var b = this.oApp && this.oApp.htOptions.I18N_LOCALE;
        if (b) {
          h += "/" + b
        }
        h += "/smart_editor2_in.css";
...
nhn.husky.SE_WYSIWYGEnterKey = jindo.$Class({
  ...
  _convertHeadSpace: function(e) {
    if (e && e.nodeType === 3) {
      ...
      if (b > 0) {
        e.nodeValue = d + a.substring(b)
      }
    }
  },
  _getValidNextSibling: function(b) {
    var a = b.nextSibling;
    if (!a) {
      return null
    } else {
  ...
  _wrapBlock: function(o) {
    var g = this.oApp.getSelection(),
      n = g.placeStringBookmark(),
      r = g.getLineInfo(),
      q = r.oStart,
      h = r.oEnd,
      p, j, m;
    if (!q.bParentBreak || g.rxBlockContainer.test(q.oLineBreaker.tagName)) {
      o.stop();
      g.deleteContents();
      if (!!q.oNode.parentNode && q.oNode.parentNode.nodeType !== 11) {
        p = this.oApp.getWYSIWYGDocument().createElement(this.sLineBreaker);
        g.moveToBookmark(n);
        g.setStartBefore(q.oNode);
        g.surroundContents(p);
        g.collapseToEnd();
        j = this.oApp.getWYSIWYGDocument().createElement(this.sLineBreaker);
        var e = this._getBlockEndNode(q.oNode, h.oNode);
        if (e === q.oNode) {
          e = h.oNode
        }
        g.setEndAfter(e);
        this._addBlankText(g);
        g.surroundContents(j);
        g.moveToStringBookmark(n, true);
        g.collapseToEnd();
        g.removeStringBookmark(n);
        g.select();
        this._addCursorHolder(p);
        if (j.lastChild !== null && j.lastChild.tagName == "BR") {
          j.removeChild(j.lastChild)
        }
        m = this._addCursorHolder(j);
        if (j.nextSibling && j.nextSibling.tagName == "BR") {
        ...
        n = g.placeStringBookmark();
        setTimeout(jindo.$Fn(function(s) {
          var t = g.getStringBookmark(s);
          if (!t) {
            return
          }
          g.moveToStringBookmark(s);
          g.select();
          g.removeStringBookmark(s)
        }, this).bind(n), 0);
        return
      }
    }
    var l = g.getStringBookmark(n, true);
    if (this.htBrowser.firefox) {
      if (l && l.nextSibling && l.nextSibling.tagName == "IFRAME") {
        setTimeout(jindo.$Fn(function(s) {
          var t = g.getStringBookmark(s);
          if (!t) {
            return
        ...
        setTimeout(jindo.$Fn(function(s) {
          this._convertHeadSpace(s)
        }, this).bind(l.nextSibling), 0);
        g.removeStringBookmark(n)
      }
    } else {
      if (this.htBrowser.ie) {
        var k = l.parentNode,
          d = false,
          b = false;
        this._firstBR2Line(k, g, h.oLineBreaker);
        if (!l || !k) {
          g.removeStringBookmark(n);
          return
        }
        setTimeout(jindo.$Fn(function() {
          this.oApp.getSelection().removeStringBookmark(n)
        }, this).bind(n), 0);
        d = (k.tagName === "U" || nhn.husky.SE2M_Utils.findAncestorByTagName("U", k) !== null);
        b = (k.tagName === "S" || k.tagName === "STRIKE" || (nhn.husky.SE2M_Utils.findAncestorByTagName("S", k) !== null && nhn.husky.SE2M_Utils.findAncestorByTagName("STRIKE", k) !== null));
        if (d || b) {
          setTimeout(jindo.$Fn(this._addTextDecorationTag, this).bind(d, b), 0);
          return
        }
        setTimeout(jindo.$Fn(this._addExtraCursorHolder, this).bind(k), 0)
      } else {
        var k = l.parentNode,
          c = this._getValidNextSibling(l);
        if (k.tagName == "SPAN" && c && c.nodeName == "BR" && c.nextSibling) {
          g.deleteContents();
          g.setEndNodes(l, h.oLineBreaker);
          var f = g.extractContents(),
            a = f.firstChild;
          k = q.oLineBreaker.parentNode;
          c = q.oLineBreaker.nextSibling;
          if (c) {
            k.insertBefore(f, c)
          } else {
            k.appendChild(f)
          ...
          this._convertHeadSpace(l.nextSibling);
          g.removeStringBookmark(n)
        }
      }
    }
  },
  _firstBR2Line: function(a, d, c) {
    while (a.firstChild && a.firstChild.nodeName === "BR") {
      var b = d._document.createTextNode("\u200B");
      a.replaceChild(b, a.firstChild);
      d.setStartBefore(c);
      d.setEndAfter(b);
      c.parentNode.insertBefore(d.extractContents(), c)
    }
  },
...
nhn.husky.SE_PasteHandler = jindo.$Class({
  ...
  _getCSSStyleRule: function(b) {
    var a = {};
    for (var d = 0, c;
      (c = b[d]); d++) {
      if (c.type === CSSRule.STYLE_RULE) {
        a[c.selectorText] = c.style
      }
    }
    return a
  },
  _extractStyle: function(e) {
    ...
    var b = e.match(this._rxStyleTag) || [];
    var f = b.join("\n");
    f = f.replace(this._rxStyleTagStrip, "");
    var d = this._getTmpDocument();
    var a = d.getElementsByTagName("HEAD")[0];
    var c = d.createElement("STYLE");
    c.innerHTML = f;
    a.appendChild(c);
    this._htStyleRule = this._getCSSStyleRule(c.sheet.cssRules);
    c.parentNode.removeChild(c)
  },
  _applyInlineStyle: function(f, c, d) {
    for (var e = 0, b = c.length, a; e < b; e++) {
      a = c[e];
      if (d || !f.style[a]) {
        f.style[a] = c[a]
      }
  ...
  _applyStyleRuleToInline: function(h, b, e) {
    var a = h.match(this._rxClassSelector) || [];
    if (a.length < 1) {
      return
    }
    ...
    for (var d = 0, f;
      (f = g[d]); d++) {
      this._applyInlineStyle(f, b);
      if (c) {
        jindo.$Element(f).removeClass(c)
      }
    }
  },
  _applyStyle: function(b) {
    var a = this._htStyleRule || {};
    for (var c in a) {
      this._applyStyleRuleToInline(c, a[c], b)
    }
    this._htStyleRule = null
  },
  _revertFontAfterPaste: function(a) {
    nhn.husky.SE2M_Utils.removeInvalidFont(a);
    if (document.documentMode < 11) {
      nhn.husky.SE2M_Utils.stripTags(a, "FONT")
    } else {
      nhn.husky.SE2M_Utils.convertFontToSpan(a)
    }
  },
  _removeLineHeightInSpan: function(c) {
    var d = jindo.$$('span[style*="line-height:"]', c);
    for (var b = 0, a;
      (a = d[b]); b++) {
      a.style.lineHeight = null
    }
  },
  $ON_EVENT_EDITING_AREA_PASTE: function(a) {
    var c = a.$value().clipboardData;
    if (!c) {
      return
    }
    var b = c.getData("text/html");
    if (!b) {
      return
    }
    this._extractStyle(b)
  },
  $ON_EVENT_EDITING_AREA_PASTE_DELAY: function() {
    var a = this.oApp.getWYSIWYGDocument().body;
    this._revertFontAfterPaste(a);
    this._applyStyle(a);
    this._removeLineHeightInSpan(a)
  }
});
nhn.husky.SE2M_ExecCommand = jindo.$Class({
  ...
  $BEFORE_EXECCOMMAND: function(a, j, g, f) {
    ...
    for (var c = 0, e = b.getNodes(), h;
      (h = e[c]); c++) {
      nhn.husky.SE2M_Utils.removeInvalidNodeInTable(h)
    }
  ...
  _checkBlockQuoteCondition_IE: function() {
    ...
    var c = false;
    var a;
    if ((g.ie && (g.nativeVersion >= 9 && g.nativeVersion <= 11) && (g.version >= 9 && g.version <= 11)) || (this.oApp.oAgent.os().winxp && g.ie && g.nativeVersion <= 8) || g.edge) {
      var f = this.oApp.getSelection();
  ...
  _removeElementAlign: function(c) {
    var e = this.oApp.getSelection(),
      a = e.getNodes(),
      b = null,
      g = new RegExp("^" + c + "$", "i");
...
nhn.husky.SE_WYSIWYGStyler = jindo.$Class({
  ...
  $ON_SET_WYSIWYG_STYLE: function(c) {
    var b = this.oApp.getSelection();
    var y = {};
    this.oApp.exec("IS_SELECTED_TD_BLOCK", ["bIsSelectedTd", y]);
    var g = y.bIsSelectedTd;
    if (b.collapsed && !g) {
      ...
      if (d && b._rxCursorHolder.test(d.innerHTML)) {
        m = b._findParentSingleSpan(d)
      }
      if (!m) {
        m = this.oApp.getWYSIWYGDocument().createElement("SPAN");
        m.innerHTML = this._sCursorHolder;
        r = true
      } else {
        if (m.innerHTML == "") {
          m.innerHTML = this._sCursorHolder
      ...
      if (r) {
        if (b.startContainer.tagName == "BODY" && b.startOffset === 0) {
          var o = b._getVeryFirstRealChild(this.oApp.getWYSIWYGDocument().body);
          var h = true;
          ...
          if (h && q.nodeType == 1 && q.tagName == "BR") {
            b.selectNode(o);
            b.collapseToStart();
            b.insertNode(m)
          } else {
            if (h && o.tagName != "IFRAME" && o.appendChild && typeof o.innerHTML == "string") {
              o.appendChild(m)
            } else {
              b.selectNode(o);
              b.collapseToStart();
      ...
      if (!!c.color) {
        b._checkTextDecoration(m)
      }
      b.selectNodeContents(r ? m : d);
      b.collapseToEnd();
      b._window.focus();
    ...
    if (g) {
      var s;
      this.oApp.exec("GET_SELECTED_TD_BLOCK", ["aTdCells", y]);
      s = y.aTdCells;
      for (var t = 0; t < s.length; t++) {
        b.selectNodeContents(s[t]);
        b.styleRange(c);
        b.select()
      }
    } else {
      var n = !!c.color;
      var u = c.fontSize || c.fontFamily;
      for (var v = 0, s = b.getNodes(), k;
        (k = s[v]); v++) {
        nhn.husky.SE2M_Utils.removeInvalidNodeInTable(k)
      }
      b.styleRange(c, null, null, u, n);
      if (jindo.$Agent().navigator().firefox) {
        var p = b.aStyleParents;
        for (var v = 0, l = p.length; v < l; v++) {
          var x = p[v];
          if (x.nextSibling && x.nextSibling.tagName == "BR" && !x.nextSibling.nextSibling) {
...
nhn.husky.SE2M_ColorPalette = jindo.$Class({
  ...
  $ON_APPLY_COLOR: function(a) {
    ...
    if (b.hasClass("husky_se2m_color_palette")) {
      this.oApp.exec("COLOR_PALETTE_APPLY_COLOR", [c, false])
    } else {
...
Shortcut.Store = {
  ...
  currentId: "",
  currentKey: "",
  searchId: function(b, c) {
    var d = false;
    jindo.$H(this.datas).forEach(function(e, f) {
      if (c == e.element) {
        b = f;
        d = true;
        jindo.$H.Break()
    ...
    if (!d && (b in this.datas)) {
      var a = b;
      while (a in this.datas) {
        a = "nonID" + new Date().getTime()
      }
      return a
    }
    return b
...
nhn.husky.MessageManager = jindo.$Class({
  ...
  $init: function(a, b) {
    ...
    c = c || a;
    this._sDefaultLocale = b || this._sDefaultLocale;
    this._setMessageMap(c, this._sDefaultLocale)
  },
  _setMessageMap: function(a, b) {
    b = b || this._sDefaultLocale;
    if (a) {
      this._oMessageMapSet[b] = a
    }
  },
  _getMessageMap: function(a) {
    return this._oMessageMapSet[a] || this._oMessageMapSet[this._sDefaultLocale] || {}
  },
  $BEFORE_MSG_APP_READY: function() {
    this.oApp.exec("ADD_APP_PROPERTY", ["$MSG", jindo.$Fn(this.getMessage, this).bind()])
  },
  getMessage: function(b, c) {
    var a = this._getMessageMap(c);
    if (a[b]) {
      return unescape(a[b])
    }
    return b
...
nhn.husky.SE2M_Utils = {
  ...
  isInvalidNodeInTable: function(b) {
    if (b && !this._rxTable.test(b.nodeName)) {
      var a;
  ...
  removeInvalidNodeInTable: function(a) {
    if (this.isInvalidNodeInTable(a) && a.parentNode) {
      a.parentNode.removeChild(a)
    }
  },
  removeInvalidFont: function(a) {
    if (!a) {
      return
    }
    this._removeInvalidFontInTable(a);
    this._removeEmptyFont(a)
  },
  _removeInvalidFontInTable: function(c) {
    var b = jindo.$$("table font", c);
    for (var a = 0, d;
      (d = b[a]); a++) {
      this.removeInvalidNodeInTable(d)
    }
  },
  _removeEmptyFont: function(d) {
    var b = jindo.$$("font", d);
    for (var a = 0, e, c;
      (e = b[a]); a++) {
      c = e.innerHTML || "";
      c = c.replace(this._rxSpaceOnly, "");
      if (!c) {
  ...
  convertFontToSpan: function(e) {
    ...
    for (var d = 0, f, a, c;
      (f = g[d]); d++) {
      a = f.parentNode;
      if (a.tagName !== "SPAN" || a.childNodes.length > 1) {
        a = b.createElement("SPAN");
        f.parentNode.insertBefore(a, f)
      }
      c = f.getAttribute("face");
      if (c) {
        a.style.fontFamily = c
      }
      c = this._htFontSize[f.getAttribute("size")];
      if (c) {
        a.style.fontSize = c
      }
      ...
      this._switchFontInnerToSpan(f, a);
      if (f.parentNode) {
        f.parentNode.removeChild(f)
      }
    }
  },
  _switchFontInnerToSpan: function(d, a) {
    var b = d.innerHTML;
    if (this._rxFontStart.test(b)) {
      for (var c;
        (c = d.firstChild);) {
        a.appendChild(c)
      }
  ...
  reviseDateFormat: function(a) {
    if (a && a.replace) {
      a = a.replace(this.rxDateFormat, "$&.")
    }
    return a
  },
  ...
  deleteProperty: function(a, b) {
    if ((typeof(a) !== "object") || (typeof(b) !== "string") || (typeof(a[b]) === "undefined")) {
      return
    }
    a[b] = undefined;
    try {
...
nhn.husky.SE2B_CSSLoader = jindo.$Class({
  ...
  loadSE2CSS: function(a, c) {
    ...
    var e = null;
    if (a) {
      e = jindo.$Fn(this.oApp.exec, this.oApp).bind(a, c)
    }
    var d = this.htOptions.sCSSBaseURI;
    var b = this.oApp && this.oApp.htOptions.I18N_LOCALE;
    if (b) {
      d += "/" + b
    }
    d += "/smart_editor2_items.css";
    nhn.husky.SE2M_Utils.loadCSS(d, e);
    return false
  }
});
if (typeof window.nhn == "undefined") {
...
nhn.husky.HuskyCore.mixin(nhn.husky.SE2M_TableEditor, {
  ...
  $ON_TE_SPLIT_COLUMN: function() {
    ...
    for (var c = 0, l = this.aSelectedCells.length; c < l; c++) {
      ...
      var a = jindo.$Agent().navigator();
      if ((a.edge && (Math.floor(a.version) === 12)) || (a.ie && (a.nativeVersion >= 9 || a.nativeVersion <= 11) && (a.version >= 9 || a.version <= 11))) {
  ...
  $ON_TE_SPLIT_ROW: function() {
    ...
    for (var s = 0, l = this.aSelectedCells.length; s < l; s++) {
      ...
      if ((j.edge && (Math.floor(j.version) === 12)) || (j.ie && (j.nativeVersion >= 9 || j.nativeVersion <= 11) && (j.version >= 9 || j.version <= 11))) {
  ...
  $ON_EVENT_EDITING_AREA_MOUSEMOVE: function(g) {
    if (this.oApp.getEditingMode() != "WYSIWYG") {
      return
    }
    var d = g.element,
      e = jindo.$Agent().navigator(),
      f = "onpointerup",
      c = "onresizeend";
    if (e.ie && d && d.tagName && (d.tagName.toUpperCase() === "TABLE")) {
      if (f in d) {
        d[f] = this._wfnOnResizeEndTable
      } else {
        if (c in d) {
          if (e.version > 8) {
            d[c] = this._wfnOnResizeEndTable
          } else {
            d[c] = this._getTableResizeEndHandler(d)
          }
        }
      }
    }
    switch (this.nStatus) {
      case this.STATUS.S_0:
        if (this._isOnBorder(g)) {
          this._showCellResizeGrip(g)
        } else {
          this._hideResizer()
        }
        break;
      case this.STATUS.MOUSEDOWN_CELL:
        var b = nhn.husky.SE2M_Utils.findClosestAncestorAmongTagNames(this._aCellName, g.element);
  ...
  _reassignCellSizes: function(A) {
    var e = new Array(2);
    e[0] = jindo.$$(">TBODY>TR>TD", A, {
      oneTimeOffCache: true
    });
    e[1] = jindo.$$(">TBODY>TR>TH", A, {
      oneTimeOffCache: true
    });
    var r = new Array(e[0].length + e[1].length);
    var d = 0;
    var q = this.parseIntOr0(A.border);
    var a = this.parseIntOr0(A.cellPadding);
    for (var u = 0; u < 2; u++) {
      for (var z = 0; z < e[u].length; z++) {
        var j = e[u][z];
        var b = jindo.$Element(j);
        var m = jindo.$Agent().navigator();
        var E, x, F, C;
        var t, h, g, G;
        var w, B;
        if (j.getComputedStyle) {
          E = parseFloat(getComputedStyle(j).paddingLeft, 10);
          x = parseFloat(getComputedStyle(j).paddingRight, 10);
          F = parseFloat(getComputedStyle(j).paddingTop, 10);
          C = parseFloat(getComputedStyle(j).paddingBottom, 10);
          t = parseFloat(getComputedStyle(j).borderLeftWidth, 10);
          h = parseFloat(getComputedStyle(j).borderRightWidth, 10);
          g = parseFloat(getComputedStyle(j).borderTopWidth, 10);
          G = parseFloat(getComputedStyle(j).borderBottomWidth, 10)
        } else {
          E = this.parseIntOr0(j.style.paddingLeft);
          x = this.parseIntOr0(j.style.paddingRight);
          F = this.parseIntOr0(j.style.paddingTop);
          C = this.parseIntOr0(j.style.paddingBottom);
          t = this.parseBorder(b.css("borderLeftWidth"), b.css("borderLeftStyle"));
          h = this.parseBorder(b.css("borderRightWidth"), b.css("borderRightStyle"));
          g = this.parseBorder(b.css("borderTopWidth"), b.css("borderTopStyle"));
          G = this.parseBorder(b.css("borderBottomWidth"), b.css("borderBottomStyle"))
        }
        var l = jindo.$Element(j).attr("width");
        var D = jindo.$Element(j).attr("height");
        if (!l && !D) {
          w = j.style.width;
          B = j.style.height
        } else {
          w = j.offsetWidth - (E + x + t + h) + "px";
          B = j.offsetHeight - (F + C + g + G) + "px"
        }
        r[d++] = [j, w, B]
      }
    }
    var k = A._nWidth,
      ...
      f = 1,
      c = 1;
    if (nhn.husky.SE2M_Utils.isNumber(k) && nhn.husky.SE2M_Utils.isNumber(o) && nhn.husky.SE2M_Utils.isNumber(p) && nhn.husky.SE2M_Utils.isNumber(v)) {
      f = p / k, c = v / o
    }
    if (A._nResizedWidth) {
      nhn.husky.SE2M_Utils.deleteProperty(A, "_nResizedWidth")
    }
    if (A._nResizedHeight) {
      nhn.husky.SE2M_Utils.deleteProperty(A, "_nResizedHeight")
    }
    for (var z = 0; z < d; z++) {
      var s = r[z];
      s[0].removeAttribute("width");
      s[0].removeAttribute("height");
      s[0].style.width = (parseFloat(s[1], 10) * f) + "px";
      s[0].style.height = (parseFloat(s[2], 10) * c) + "px"
    }
    var y = jindo.$Element(A);
    A._nWidth = y.width(), A._nHeight = y.height();
    A.removeAttribute("width");
    A.removeAttribute("height");
    A.style.width = "";
    A.style.height = ""
  },
  _fnOnMouseDownResizeCover: function(c) {
    this.bResizing = true;
  ...
  _fnOnMouseMoveResizeCover: function(b) {
  ...
  _fnOnMouseUpResizeCover: function(h) {
  ...
  _fnOnResizeEndTable: function(a) {
    var b = jindo.$Event(a);
    var c = b.element;
    this._markResizedMetric(c)
  },
  _getTableResizeEndHandler: function(a) {
    return jindo.$Fn(this._markResizedMetric, this).bind(a)
  },
  _markResizedMetric: function(b) {
    if (!b || (b.tagName.toUpperCase() !== "TABLE")) {
      return
    }
    var a = jindo.$Element(b);
    b._nResizedWidth = a.width(), b._nResizedHeight = a.height()
  },
  $ON_CLOSE_QE_LAYER: function(c) {
    var h = c ? c.element : null,
      f = h ? jindo.$Element(h) : null,
      e = ["q_open_table_fold", "q_open_table_full"],
      b, d, a = e.length,
      g = false;
    if (f) {
      for (d = a; d--;) {
  ...
  _isOnBorder: function(j) {
    this.htResizing.nResizeMode = 0;
    this.htResizing.elCell = j.element;
    if (!this._rxCellNames.test(j.element.tagName)) {
      return false
    }
    ...
    this.htResizing.htEPos = j.pos(true);
    ...
    if (jindo.$Agent().navigator().ie || jindo.$Agent().navigator().safari) {
      e = this.htResizing.nBorderSize + this.nDraggableCellEdge;
      d = this.nDraggableCellEdge
    } else {
      e = this.nDraggableCellEdge;
      d = this.htResizing.nBorderSize + this.nDraggableCellEdge
    }
    var g = this.htResizing.elCell.clientWidth,
      h = this.htResizing.elCell.clientHeight;
    nRightBorderCriteria = g - this.htResizing.htEPos.offsetX, nBottomBorderCriteria = h - this.htResizing.htEPos.offsetY;
    if (this.htResizing.htEPos.offsetY <= e) {
      var a = false,
        c = this.htResizing.elCell.parentNode;
      var b = c.parentNode;
      var f = jindo.$$("tr", b, {
        oneTimeOffCache: true
      });
    ...
    if (nBottomBorderCriteria <= d) {
      this.htResizing.nBorderTopPos = this.htResizing.elCell.offsetHeight + e - 1;
      this.htResizing.nResizeMode = 2
    }
    if (this.htResizing.htEPos.offsetX <= e) {
    ...
    if (nRightBorderCriteria <= e) {
      this.htResizing.nBorderLeftPos = this.htResizing.elCell.offsetWidth + e - 1;
  ...
  _insertColumnAfter: function() {
    ...
    for (var l = 0, o = this.htMap[0].length; l < o; l++) {
      ...
      for (var m = this.htSelectionEPos.x; m >= this.htSelectionSPos.x; m--) {
        ...
        k.insertBefore(b, g);
        if ((c.edge && (Math.floor(c.version) === 12)) || (c.ie && (c.nativeVersion >= 9 || c.nativeVersion <= 11) && (c.version >= 9 || c.version <= 11))) {
...
nhn.FindReplace = jindo.$Class({
  ...
  findNext: function(a, b, d, g) {
    ...
    if (this.window.find) {
      var j = false;
      if (this.document.body.contentEditable === "false") {
        return window.find(a, b, d, j, g)
      } else {
