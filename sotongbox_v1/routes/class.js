  ...
  route.get('/Inner/:id/courseEvaluation', function(req, res) {
    ...
    pool.getConnection(function(err, connection) {
      ...
      var first_sql = "select m_no, c_no, b_me_no," +
        ...
        " (select count(bo.b_title) from board bo, b_write b, class c  where b.c_no = c.c_no and bo.b_no = b.b_no and b.c_no = '" + c_no + "' ) as commentNum," +
        " (select round(sum(substr(b_title, 2)) / commentNum, 0)  from board bo, b_write b, class c where b.c_no = c.c_no and bo.b_no = b.b_no and b.c_no = '" + c_no + "') as starGrade" +
        " from b_write b" +
        " where b_me_no = 'bm0001'" +
        " and m_no = (" +
        " select m_no" +
        " from member" +
        " where m_no = b.m_no and c_no = '" + c_no + "')" +
        " order by (select date_format(b_register, '%y-%m-%d %H:%i:%s') from board where b_no = b.b_no)"
      connection.query(first_sql, function(err, row) {
